...
function validateForm() {
    ...
    if (fname === "") {
        error[3].innerHTML = "Họ tên không được bỏ trống";
        checkFname = false;
    } else {
        error[3].innerHTML = "";
        if (rgxFname.test(fname)) {
            checkFname = true;
        } else {
            error[3].innerHTML = "Họ tên không chứa số và ký tự đặc biệt và có độ dài lớn hơn 2.";
            checkFname = false;
        }
    }
    ...
    if (checkFname && checkAddress && checkEmail && checkPasswd && checkPasswd && checkUname && checkInterest) {
        checkForm = true;
    }
    return checkForm;
}